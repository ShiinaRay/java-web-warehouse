...
public class IsExistDao1 {//place  
	public int IsExist(Place place)//1:不存在相同 直接插入  0：存在相同 增加数量 -1：系统异常
	{
		...
		try {
			...
			// c.发送sql
			String sql = "select gno,place from place";
			...
			while(rs.next()) {
				...
				if(Integer.parseInt(place.getGno())==Integer.parseInt(rs.getString(1)) &&  place.getPlace().equals(rs.getString(2))  ) {//String 不能直接相等
					flag = 0;
					break;
				}
        }	
