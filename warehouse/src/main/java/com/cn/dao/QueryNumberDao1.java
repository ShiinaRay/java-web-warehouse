...
public class QueryNumberDao1 {//place
//	public int login(String name,String pwd)//1:登录成功   0：登录失败（用户名或密码有误）  -1：系统异常
	public int QueryNumber(Place place)//1:登录成功   0：登录失败（用户名或密码有误）  -1：系统异常
	{
		...
		try {
			...
			// c.发送sql
			String sql = "select wnumber from place where gno=? and place=?";
